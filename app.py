...
# --- Configuration ---
MODEL_URL = "https://drive.google.com/file/d/1WXesbCqdDGLVB_djSQ_kl4szj4hRdilJ/view?usp=sharing"
MODEL_FILENAME = "pipeline_rf_protocol2.pkl"  # Updated model filename
...
@st.cache_resource
def load_model():
    local_model_path = "pipeline_rf_protocol2.pkl" # Consistent variable name
    if not os.path.exists(local_model_path):
        st.info(f"Downloading model from {MODEL_URL}...")
        try:
            response = requests.get(MODEL_URL, stream=True)
            response.raise_for_status()  # Raise an exception for bad status codes
            with open(local_model_path, "wb") as f: # Save with the correct name
    ...
    try:
        pipeline_rf, saved_columns = joblib.load(local_model_path) # Load with the correct name
...
with st.sidebar:
    ...
    color_name2 = st.selectbox("Color2 (optional)", color_options_list)
    ...
    if st.button("Predict Adoption Speed"):
        if not description.strip():
            ...
            if not isinstance(fee, (int, float)):
                st.error("Adoption Fee must be a number.")
                st.stop()
            if not isinstance(video_amt, int):
                st.error("Number of Videos must be an integer.")
                st.stop()
            if not isinstance(photo_amt, int):
                st.error("Number of Photos must be an integer.")
                st.stop()
